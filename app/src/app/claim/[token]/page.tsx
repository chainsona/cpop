...
export default function ClaimPage() {
  ...
  const handleClaim = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to claim POAP');
      }
      ...
      setClaimStatus(prevStatus =>
        prevStatus ? { ...prevStatus, claimed: true, message: 'POAP claimed successfully' } : null
      );

      toast.success('POAP claimed successfully!');
    } catch (err) {
      console.error('Error claiming POAP:', err);
      toast.error(err instanceof Error ? err.message : 'Failed to claim POAP');
    } finally {
      setIsClaiming(false);
    }
  };
  ...
  if (error || !claimStatus) {
    return (
      <div className="flex flex-col sm:items-center sm:justify-center" style={{ height: viewportHeight }}>
    ...
    return (
      <div className="flex flex-col sm:items-center sm:justify-center" style={{ height: viewportHeight }}>
        <div className="flex-1 sm:flex-initial flex flex-col items-center justify-center p-6 sm:max-w-md sm:border sm:rounded-lg sm:shadow-sm sm:bg-white sm:p-8">
          ...
          <CardTitle className="text-xl mb-2">POAP Already Claimed</CardTitle>
          <CardDescription className="text-center mb-6">
            This POAP has already been claimed.
          </CardDescription>
          
          {claimStatus.poap && (
            <div className="flex flex-col items-center space-y-4 mb-8">
              <div className="relative w-32 h-32 rounded-xl overflow-hidden">
                {claimStatus.poap.imageUrl ? (
                  <Image
                    src={claimStatus.poap.imageUrl}
              ...
              <div className="text-center">
                <h3 className="font-semibold text-lg">{claimStatus.poap.title}</h3>
                <p className="text-sm text-neutral-600">{claimStatus.poap.description}</p>
              </div>
            </div>
          )}
  ...
  // Main claim flow
  return (
    <div className="flex flex-col sm:justify-center sm:items-center" style={{ height: viewportHeight }}>
      <div className="flex-1 overflow-y-auto w-full sm:flex-initial sm:max-w-md sm:border sm:rounded-lg sm:shadow-sm sm:bg-white sm:overflow-hidden">
        <div className="p-6 pb-32 sm:pb-6">
          <div className="text-center mb-6">
            <h1 className="text-2xl font-bold">Claim Your POAP</h1>
          </div>
          ...
          {claimStatus?.poap && (
            <div className="flex flex-col">
              <div className="flex flex-col items-center space-y-6 mb-8">
                ...
                <div className="text-center">
                  <h3 className="font-semibold text-xl">{claimStatus.poap.title}</h3>
                  <p className="text-neutral-600 mt-1">{claimStatus.poap.description}</p>
                  {claimStatus.poap.creatorName && (
                    <p className="text-sm text-neutral-500 mt-2">
                      Created by {claimStatus.poap.creatorName}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <p className="text-sm text-neutral-500">
                  Make sure this is a valid Solana wallet address. The POAP will be minted directly
                  to this address.
                </p>
              </div>
            </div>
