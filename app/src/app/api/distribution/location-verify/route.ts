...
/**
 * API endpoint to verify a user's location for a location-based POAP claim
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const existingClaim = await prisma.pOAPClaim.findUnique({
      where: {
        poapId_walletAddress: {
          poapId: distributionMethod.poapId,
          walletAddress,
        },
      },
    });
    ...
    return NextResponse.json({
      ...
      poap: {
        id: distributionMethod.poap.id,
        title: distributionMethod.poap.title,
