...
import { POAPConfigLayout } from '@/components/poap/poap-config-layout';
import { POAPImageDisplay } from '@/components/poap/poap-image-display';
import { TokenWarning } from '@/components/poap/token-warning';
import { POAPInfoDisplay } from '@/components/poap/poap-info-display';
import { POAPAnalytics, AnalyticsData } from '@/components/poap/analytics/poap-analytics';
import { POAPClaimSection } from '@/components/poap/poap-claim-section';
...
export default function POAPDetailPage() {
  ...
  const [metadataStatus, setMetadataStatus] = useState<'complete' | 'incomplete' | 'partial'>(
    'incomplete'
  );
  const [metadataSummary, setMetadataSummary] = useState('No metadata configured');
    ...
    const fetchData = async () => {
      try {
        ...
        if (!poapResponse.ok) {
          ...
          if (poapResponse.status === 401 || poapResponse.status === 403) {
            // Handle unauthorized/forbidden - but still try to get public data
            const publicResponse = await fetch(`/api/poaps/${id}/public`);
            if (publicResponse.ok) {
              const publicData = await publicResponse.json();
              setPoap(publicData.poap);

              // Set page title based on POAP title
              if (publicData.poap?.title) {
                setPageTitle(publicData.poap.title);
              }
            } else {
              throw new Error('Unable to access POAP details');
            }
          } else {
          ...
          if (isAuthenticated) {
            ...
            const promises = [
              fetch(`/api/poaps/${id}/distribution`),
              fetch(`/api/poaps/${id}/attributes`),
              fetch(`/api/poaps/${id}/settings`),
            ];
            ...
            if (attributesResponse.ok) {
              ...
              if (data.attributes) {
                ...
                // Set metadata status based on available information
                if (hasEventType && (hasLocation || hasPlatform) && hasOrganization) {
                  setMetadataStatus('complete');
                  setMetadataSummary(
                    hasArtists ? 'All metadata configured' : 'Essential metadata configured'
                  );
                } else if (hasEventType || hasLocation || hasPlatform || hasOrganization) {
                  setMetadataStatus('partial');
                  setMetadataSummary('Basic metadata available');
                } else {
                  setMetadataStatus('incomplete');
                  setMetadataSummary('Basic metadata missing');
                }
              }
            }
            ...
            try {
              const publicAttributesResponse = await fetch(`/api/poaps/${id}/attributes/public`);
  ...
  useEffect(() => {
    if (!isAuthenticated || !poap) return;
    ...
    return (
      <div className="container mx-auto py-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h2 className="text-xl font-semibold mb-2">{error || 'Unable to load POAP details'}</h2>
  ...
  const palette = getColorPaletteForId(poap.id);
  const statusDisplay = getStatusDisplay(poap.status);
  const isOwner =
    isAuthenticated &&
    walletAddress &&
    poap.creator &&
    poap.creator.walletAddress === walletAddress;
    ...
    <div className="px-4 sm:px-6 py-6 pb-16">
      ...
      <div className="max-w-6xl mx-auto mb-4">
        <Button variant="ghost" size="sm" asChild className="mb-4">
          <Link href="/poaps" className="text-neutral-600 hover:text-neutral-900">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="md:col-span-2">
            ...
            {isAuthenticated && !isOwner ? (
              ...
              <div className="mb-8">
                <POAPClaimSection poapId={poap.id} title={poap.title} />
              </div>
            ) : isOwner ? (
              ...
              <>
                ...
                <POAPConfigLayout
                  ...
                  tokenStatus={poap.token ? 'complete' : 'incomplete'}
                  tokenSummary={poap.token ? 'Token configured' : 'No token configured'}
                  settingsStatus={settingsStatus}
                  settingsSummary={settingsSummary}
                  className="mb-8"
                />

                {/* POAP tabs navigation - only for authenticated users */}
                <div className="mb-8">
                  <POAPTabNav poapId={poap.id} />
                </div>
