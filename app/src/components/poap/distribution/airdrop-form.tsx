...
export function AirdropForm({ id, onSuccess }: AirdropFormProps) {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (importedAddresses.length === 0) {
        ...
        toast.success(`Imported ${importedAddresses.length} addresses${importedAddresses.length !== uniqueAddresses.length - existingAddressesList.length ? ` (${uniqueAddresses.length - existingAddressesList.length} unique)` : ''}`);
      }
    } catch (error) {
      console.error('Error importing file:', error);
      setValidationErrors([`Failed to import file: ${error instanceof Error ? error.message : 'Unknown error'}`]);
    } finally {
  ...
  const calculateCost = (recipientCount: number) => {
    ...
    const compressedPriorityFee = (transactionCount * computeUnitLimit * priorityFeeEstimate) / (MICRO_LAMPORTS_PER_LAMPORT * 1e9);
    ...
    const normalPriorityFee = (transactionCount * computeUnitLimit * priorityFeeEstimate) / (MICRO_LAMPORTS_PER_LAMPORT * 1e9);
        ...
        <div className="space-y-6">
          ...
          <div className="max-w-md">
            <div className="flex justify-between items-center mb-2">
              <Label htmlFor="addresses">Solana Addresses <span className="text-red-500">*</span></Label>
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" size="icon" className="h-8 w-8 mr-1 hover:bg-neutral-100">
                        <HelpCircle className="h-4 w-4 text-neutral-600" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent side="left" className="max-w-sm p-4 bg-white text-neutral-900 shadow-lg border border-neutral-200">
            ...
            <div className="bg-white p-4 rounded-lg border border-neutral-200">
              ...
              <div>
                ...
                <div className="flex gap-3 items-center mt-2">
                  ...
                  <Badge variant="outline" className="ml-1">Default: Now</Badge>
                </div>
                <p className="text-sm text-neutral-600 mt-2">
                  Distribution will begin at this date and time. Choose now for immediate distribution.
                </p>
              </div>
            </div>
                      ...
                      return (
                        <Alert variant="destructive" className="mt-3 py-2">
                          ...
                          <AlertDescription className="text-xs">
                            Your budget ({budget.toFixed(6)} SOL) is lower than the estimated cost ({compressedCost} SOL).
                          </AlertDescription>
                        </Alert>
                      );
                    } else {
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent className="w-80 p-3 text-sm">
                      <p className="mb-2"><strong>Compute Unit Limit:</strong> Maximum computation units allowed for each transaction. Higher values allow more complex transactions.</p>
                      <p><strong>Priority Fee:</strong> Additional fee to prioritize your transaction during network congestion. 1 microlamport = 0.000001 lamports.</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-sm text-neutral-600">{computeUnitLimit.toLocaleString()}</span>
                  </div>
                  <Slider 
                    id="computeUnitLimit"
                    min={300000} 
          ...
          <div className="space-y-4">
                ...
                return (
                  <div className="space-y-4">
                    <dl className="space-y-2 bg-white rounded-md p-3 border border-blue-100">
                      ...
                      {budget !== undefined && parseFloat(compressedCost) > budget && (
                        <div className="flex justify-between py-1 border-t border-blue-100 bg-red-50 px-2 -mx-2 rounded">
                          ...
                          <dd className="font-semibold text-red-700">{budget.toFixed(6)} SOL (insufficient)</dd>
                        </div>
                      )}
                    </dl>
                    ...
                    <div className="bg-blue-100 p-3 rounded-md text-sm text-blue-800">
                      ...
                      <p className="mb-2">
                        Traditional airdrops require creating an Associated Token Account (ATA) for each recipient (~0.002 SOL each).
                      </p>
                      <p>
                        Airdrop with ZK Compression avoids these costs by using compressed token accounts while maintaining the same security as regular Solana tokens.
                      </p>
                    </div>
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button 
                ...
                disabled={isSubmitting || isMinting || (budget !== undefined && budget < parseFloat(calculateCost(parsedAddresses.length).compressedCost))}
              >
                {isSubmitting ? 'Creating...' : budget !== undefined && budget < parseFloat(calculateCost(parsedAddresses.length).compressedCost) ? 'Insufficient Budget' : 'Create Airdrop Distribution'}
              </Button>
            </div>
            ...
            {isMinting && (
              <div className="mt-6 bg-blue-50 p-4 rounded-lg flex items-center gap-3 border border-blue-100 animate-pulse">
                ...
                <div>
                  ...
                  <p className="text-blue-600 text-sm">The tokens will be minted directly to recipient wallets.</p>
                </div>
              </div>
            )}
          </div>
