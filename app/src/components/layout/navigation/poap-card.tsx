...
interface POAPCardProps {
  poap: POAP;
  onClick: (poap: POAP) => void;
  type?: 'compact' | 'full';
}

export function POAPCard({ poap, onClick, type = 'full' }: POAPCardProps) {
  const statusColors = {
      ...
      >
        <div className="h-16 w-16 rounded-lg bg-neutral-100 overflow-hidden flex-shrink-0 shadow-sm group-hover:shadow transition-shadow">
          <img
            ...
            onError={e => {
              (e.target as HTMLImageElement).src = 'https://via.placeholder.com/64?text=POAP';
            }}
          />
        </div>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className={`text-xs px-1.5 py-0.5 rounded-full ${statusColors[poap.status]}`}>
              {poap.status}
            </span>
          </div>
        </div>
    ...
    >
      <div className="h-20 w-20 rounded-lg bg-neutral-100 overflow-hidden flex-shrink-0 shadow-sm group-hover:shadow group-hover:ring-1 group-hover:ring-blue-200 transition-all">
        <img
          ...
          className="h-full w-full object-cover"
          onError={e => {
            (e.target as HTMLImageElement).src = 'https://via.placeholder.com/80?text=POAP';
          }}
        />
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-2">
          <span className={`text-xs px-2 py-0.5 rounded-full ${statusColors[poap.status]}`}>
            {poap.status}
          </span>
        </div>
      </div>
