...
export function POPForm({ mode = 'create', initialData, onSuccess }: POPFormProps) {
  ...
  const form = useForm<any>({
    resolver: zodResolver(popFormSchema) as any,
    defaultValues: {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      if (!data.imageUrl || data.imageUrl.trim() === '') {
        data.imageUrl = 'https://placehold.co/600x400?text=POP+Image';
      }
      ...
      const endpoint = mode === 'create' ? '/api/pops' : `/api/pops/${initialData?.id}`;
      ...
      if (!response.ok) {
        ...
        throw new Error(responseData.error || responseData.message || 'Failed to process POP');
      }
      ...
      if (mode === 'edit' && initialData?.id) {
        ...
        if (hasMetadataChanges) {
          ...
          localStorage.setItem(`pop-${initialData.id}-metadata-outdated`, 'true');
        }
      }
