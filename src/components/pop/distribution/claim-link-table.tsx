...
export const ClaimLinkTable = ({
  ...
  const shareClaimLink = (token: string, claimUrl: string) => {
    ...
    if (navigator.share) {
      navigator
        .share({
          title: 'POP Claim Link',
          text: 'Claim your POP using this link',
          url: claimUrl,
        })
        ...
        <div className="bg-white rounded-lg border border-neutral-200 p-4 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-2xl font-bold text-purple-600">
            {claimLinks && claimLinks.length > 0
              ? (
                  (claimLinks.filter(link => link.claimed).length / claimLinks.length) *
                  100
                ).toFixed(2)
              : 0}
            %
                          ...
                          {!link.claimed && (
                            <>
                              ...
                              <Button
                                ...
                                size="sm"
                                className="h-7 p-0 w-7"
                                onClick={() => window.open(getClaimUrl(link.token), '_blank')}
                              >
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center gap-1">
                              <span className="text-xs">
                                {link.claimedAt ? new Date(link.claimedAt).toLocaleString() : '—'}
                              </span>
                            </div>
                            ...
                            {link.claimedByWallet && (
                              <div className="flex items-center gap-1">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    link.claimedByWallet && copyToClipboard(link.claimedByWallet)
                                  }
                                >
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-neutral-50 border-t border-neutral-200">
            <div>
              <p className="text-sm text-neutral-700">
                Showing{' '}
                <span className="font-medium">{(claimLinksPage - 1) * claimLinksPageSize + 1}</span>{' '}
                to{' '}
                <span className="font-medium">
                  {Math.min(claimLinksPage * claimLinksPageSize, claimLinks.length)}
                </span>{' '}
                of <span className="font-medium">{claimLinks.length}</span> links
              </p>
            </div>
            <div className="flex space-x-2">
              ...
              <Button
                ...
                disabled={claimLinksPage * claimLinksPageSize >= claimLinks.length}
              >
                Next
              </Button>
            </div>
