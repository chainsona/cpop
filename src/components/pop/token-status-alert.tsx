...
import { POPMintModal } from './pop-mint-modal';
...
/**
 * Component that displays the token minting status for a POP
...
export function TokenStatusAlert({
  ...
  const mintTokens = async () => {
    ...
    const result = await mintPOPTokens({
      popId,
      authenticate,
      isAuthenticated,
  ...
  const updateTokenMetadata = async () => {
    try {
      ...
      const response = await fetch(`/api/pops/${popId}/token/update-metadata`, {
        method: 'POST',
        headers: {
      ...
      <div className="bg-purple-50 rounded-lg p-4 flex gap-3 items-start mb-6 shadow-sm">
        ...
        <div className="flex-1">
          ...
          <p className="text-purple-700 text-sm mb-3">
            Your POP details have changed since the token metadata was last updated
  ...
  // Token exists - show success message
    ...
    alertContent = (
      <div className="bg-green-50 rounded-lg p-4 flex gap-3 items-start mb-6 shadow-sm">
        ...
        <div className="flex-1">
          <h3 className="font-medium text-green-800 mb-1">POP Token is ready</h3>
          <p className="text-green-700 text-sm">
            Your POP token has been successfully minted and is ready for distribution.
          </p>
        </div>
      </div>
    );
  ...
  // No token and has distribution methods - show warning
    ...
    alertContent = (
      <div className="bg-yellow-50 rounded-lg p-4 flex gap-3 items-start mb-6 shadow-sm">
        ...
        <div className="flex-1">
          <h3 className="font-medium text-yellow-800 mb-1">POP Token not minted yet</h3>
          <p className="text-yellow-700 text-sm mb-3">
            You've created distribution methods, but the POP Token is not minted yet. Users won't
            be able to claim POPs until the token is minted.
          </p>
          <Button
            size="sm"
