...
export function Header() {
  ...
  // Check if we're on a POP detail page and get the title directly from URL
  useEffect(() => {
    const popMatch = pathname.match(/\/pops\/([^\/]+)/);

    if (popMatch && popMatch[1]) {
      // We're on a POP page (main or subpage), try to get the POP title
      const fetchPopTitle = async () => {
        try {
          ...
          console.error('Error fetching POP title:', error);
        }
      };

      fetchPopTitle();
    } else {
      // Not on a POP page, reset the title
      setCurrentPopTitle(null);
    }
  }, [pathname]);

  // Determine what title to display
  const displayTitle = currentPopTitle || pageTitle || 'Proof of Participation';
