...
/**
 * This is a completely public version of the Solana Pay endpoint
 * It doesn't use any middleware and should be accessible without authentication
 */
...
export async function OPTIONS() {
  ...
  console.log('[Public Solana Pay] Handling OPTIONS preflight request');
  return response;
}
...
export async function GET(request: Request, { params }: { params: Promise<Params> }) {
  ...
  try {
    console.log('[Public Solana Pay] GET request for token:', token);
    ...
    console.error('[Public Solana Pay] Error in GET handler:', error);
...
export async function POST(request: Request, { params }: { params: Promise<Params> }) {
  ...
  try {
    ...
    console.log(
      '[Public Solana Pay] POST request for token:',
      token,
      'from wallet:',
      walletAddress
    );
    ...
    // Validate claim link
    if (!claimLink) {
      const errorResponse = NextResponse.json({ error: 'Claim link not found' }, { status: 404 });
    ...
    if (claimLink.claimed) {
      const errorResponse = NextResponse.json(
        { error: 'This POP has already been claimed' },
        { status: 403 }
      );
    ...
    if (!popTokens || popTokens.length === 0 || !popTokens[0].mintAddress) {
      ...
      if (!tokenResult.success || !tokenResult.mintAddress) {
        const errorResponse = NextResponse.json(
          { error: 'Failed to create POP token' },
          { status: 500 }
        );
    ...
    try {
      ...
      console.error('[Public Solana Pay] Error generating transaction:', error);
    ...
    console.error('[Public Solana Pay] Error in POST handler:', error);
