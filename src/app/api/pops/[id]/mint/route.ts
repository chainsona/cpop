...
async function mintHandler(request: Request, { params }: { params: Promise<Params> }) {
  try {
    ...
    try {
      ...
      mintAddress = await mintCompressedTokens(pop as PopData);
...
async function mintCompressedTokens(pop: PopData): Promise<string> {
  if (!RPC_ENDPOINT) {
    throw new Error('Missing Solana RPC endpoint configuration');
  }
  ...
  const metadata: TokenMetadata = {
    ...
    additionalMetadata: [
      ['description', pop.description],
      ['event_id', pop.id],
      ['created_at', pop.createdAt.toISOString()],
    ],
  };
