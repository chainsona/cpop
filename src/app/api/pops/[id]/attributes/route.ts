...
export async function POST(request: Request, { params }: { params: Promise<Params > }) {
  try {
    const { id  } = await params;
    console.log('Creating attributes for POP ID:', id);
    ...
    if (!pop) {
      console.error('POP not found with ID:', id);
      return NextResponse.json({ 
        error: 'POP not found',
        message: `No POP exists with ID: ${id}`,
        details: 'Make sure you have created a POP first before adding attributes'
      }, { status: 404 });
    }
    
    console.log('Found POP:', pop.id, pop.title);

    // Check if attributes already exist for this POP
    const existingAttributes = await prisma.attributes.findUnique({
      where: { popId: id },
    });

    if (existingAttributes) {
      console.log('Attributes already exist for POP:', id);
      return NextResponse.json(
        { 
          error: 'Attributes already exist for this POP',
...
export async function PATCH(request: Request, { params }: { params: Promise<Params > }) {
  try {
    ...
    // Check if attributes exist for this POP
    const existingAttributes = await prisma.attributes.findUnique({
      where: { popId: id },
    });
