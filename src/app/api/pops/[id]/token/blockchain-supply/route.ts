...
// GET endpoint to fetch token supply from blockchain for a POP
async function getTokenBlockchainSupplyHandler(
  ...
  try {
    ...
    if (!token || !token.mintAddress) {
      return NextResponse.json({ error: 'Token not found for this POP' }, { status: 404 });
    }
    ...
    try {
      ...
      let totalSupply = 0; // We don't track supply in the PopToken model anymore
