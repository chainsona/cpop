...
async function ensurePopTokenExists(
  ...
  try {
    ...
    return {
      success: false,
      message:
        error instanceof Error ? error.message : 'Unknown error checking/creating POP token',
    };
  }
}
...
export async function POST(request: Request, { params }: { params: Promise<Params> }) {
  try {
    ...
    if (!popTokens || popTokens.length === 0 || !popTokens[0].mintAddress) {
      // Instead of returning an error, try to create the POP token
      console.log('POP token not found, attempting to create it');
      ...
      if (!tokenResult.mintAddress) {
        return NextResponse.json(
          {
            error: 'POP token creation succeeded but mint address is missing',
          },
          { status: 500 }
        );
      }
